...
void robot_wrench() {
    ...
    Eigen::Vector3d f_w = Eigen::Vector3d(deg_to_rad(-30), 0, 0);
    ...
    Eigen::Matrix3d r_ws = rotation_matrix_from_euler_yzx(Eigen::Vector3d(deg_to_rad(60), deg_to_rad(-60), 0));

    Eigen::Vector3d f_s = r_ws.transpose() * f_w;
    Eigen::Vector3d m_w = r_ws * m_s;

    std::cout << f_s.transpose() << std::endl << m_w.transpose() << std::endl;
}
